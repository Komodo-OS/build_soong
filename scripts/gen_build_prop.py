...
def parse_args():
  ...
  config["KomodoDesc"] = config["BuildDesc"]
  config["KomodoDevice"] = config["DeviceName"]
...
def generate_build_info(args):
  ...
  if config["BuildVariant"] == "user":
    ...
    print(f"ro.build.display.id?={config['KomodoDesc']}")
  ...
  print(f"org.komodo.device={config['KomodoDevice']}")
